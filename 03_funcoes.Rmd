...
```{r}
pode_autoescola <- function(idade, emancipado = FALSE) {
  if(idade >= 18) {
...
wt_mean <- function(x, w) {
  if (length(x) != length(w)) {
    stop("`x` e `w` devem ter o mesmo tamanho", call. = FALSE)
  }
  sum(w * x) / sum(w)
}
```
...
wt_mean <- function(x, w, na.rm = FALSE) {
  if (!is.logical(na.rm)) {
    stop("`na.rm` deve ser lógico")
  }
  if (length(na.rm) != 1) {
    stop("`na.rm` deve ter tamanho 1")
  }
  if (length(x) != length(w)) {
    stop("`x` e `w` devem ter o mesmo tamanho", call. = FALSE)
  }
